    ...
    public static PingYinModel GetTotalPingYin(string str)
    {
        ...
        for (int i = 0; i < chs.Length; i++)
        {
            ...
            if (ChineseChar.IsValidChar(ch))
            {
                ...
                pinyins = cc.Pinyins.Where(x => !string.IsNullOrWhiteSpace(x)).ToList();
            }
            else
            {
            ...
            //去除声调，转小写
            pinyins = pinyins.ConvertAll(x => Regex.Replace(x, @"\d", "").ToLower());
            //去重
            pinyins = pinyins.Where(x => !string.IsNullOrWhiteSpace(x)).Distinct().ToList();
            if (pinyins.Any())
            {
        ...
        foreach (var pinyins in totalPingYins)
        {
            ...
            if (result.TotalPingYin.Count <= 0)
            {
                ...
                result.FirstPingYin = items.ConvertAll(p => p[..1]).Distinct().ToList();
            }
            else
            {
                //全拼循环匹配
                ...
                {
                    newFirstPingYins.AddRange(items.Select(item => firstPingYin + item[..1]));
                }
